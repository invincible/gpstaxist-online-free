...
public class Data extends TimerTask {
	...
	enum types {
		A_OK, R_OPEN, R_DRVSTOPSID, A_DRVSTOPSID, R_ORDER, 
		A_ORDER, R_DRVSTATEID, A_DRVSTATEID, A_DRVSTATE, 
		A_INFO, R_FREERUN, R_TARIF, A_TARIF, 
	...
	private String processRDrvstopsID(Command comm) {
		...
		try {
			// получаем кол-во стоянок
			mysql.prepare("select count(*) as stopscount from refstops;");
			mysql.queryPrep();mysql.next();
			int stopscount = mysql.getInt("stopscount");
			// получаем id стоянки для текущего позывного, чтобы потом * пририсовать , где он отмечен
			mysql.prepare("select stopid from drivershift where endtime is NULL and sign=? limit 1;");
			mysql.setString(1, comm.sign);
			mysql.queryPrep();mysql.next();
			String signstopid = mysql.getString("stopid");
			
			//создаем массив для стоянок и кол-ва машин 
			String [] stopsid       = new String [stopscount]; // id стоянок
			String [] stopsname     = new String [stopscount]; // названия стоянок
			String [] stopcarscount = new String [stopscount]; // кол-во водителей на стоянках
			
			mysql.prepare("select num,name from refstops order by num;");
			mysql.queryPrep();
			for (int i = 0; i < stopscount; i++) 
			 { mysql.next();
			   stopsid [i]   = mysql.getString("num");
			   stopsname [i] = mysql.getString("name"); 
			 }
			for (int i = 0; i < stopscount; i++) 
			 {	// получаем кол-во водителей на стоянках 
				mysql.prepare("select count(*) as carscount from drivershift where endtime is NULL and stopid=? limit 1;");
				mysql.setString(1, stopsid [i]);
				mysql.queryPrep();
				mysql.next();
				stopcarscount [i] = mysql.getString("carscount");
			 }
			...
			String out = "0\n0\nA_DRVSTOPSID\n";
			// while (mysql.next()) {
			//	out += "id:" + mysql.getInt("num") + "|stop:"+ mysql.getString("name") + "|";
			//System.out.println("signstopid=" + signstopid.toString());
			String zvezda = "";
			for (int i = 0; i < stopscount; i++) 
			{
   			//System.out.println("signstopid="+stopsid [i].toString()+" " + signstopid.toString());
  			  if ( stopsid [i].equals(signstopid))  { zvezda="*"; }	else  {zvezda="";  }	// ставим * перед именем, если там отмечен
			  out +="id:" + stopsid [i] + "|stop:"+ zvezda +stopsname [i] +" ="+ stopcarscount [i] + "|";	
			}	
	...
	public String processRDrvstateID(Command comm) {
		try {
			...
			while (mysql.next())
			{
				if (stateid==(mysql.getInt("num")))  
				{ zvezda = "*";}
				else
				{zvezda = "";};
				
				out += "id:" + mysql.getInt("num") + "|state:"+zvezda.toString()
