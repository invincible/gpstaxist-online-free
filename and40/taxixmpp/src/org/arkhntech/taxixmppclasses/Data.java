...
public class Data extends TimerTask {
	...
	public String processRFreerun(Command comm) {
		try {
			...
			//по просьбе казани //mysql.prepare("select orders.route as route,orders.meet as meet,hour(orders.pretime)as hr,minute(orders.pretime) as mn,"
			//по просьбе казани /		+ "orders.preorder,orders.orderstate as orderstate,orders.num,orders.street,orders.house,orders.porch,"
			//по просьбе казани /		+ "orders.addressfrom,orders.streetto,orders.houseto,orders.addressto from orders,drivershift where "
			//по просьбе казани /		+ " orders.drivershift=drivershift.num and drivershift.sign=? ");
			//по просьбе казани /
			...
			if (!hasOrder && !cfg.only_disp) {
				//по просьбе казани /commstr = "select route as route,meet as meet,hour(pretime)as hr,minute(pretime) as mn,preorder,num,street,"
				//по просьбе казани /		+ "house,porch,addressfrom,streetto,houseto,addressto from orders where "
				//по просьбе казани /		+ " orderstate=0 and (preorder=0 or ((pretime-interval "
				//по просьбе казани /		+ cfg.pretime
				//по просьбе казани /		+ " minute) < now()))and not (street is null) and length(street)>3 and ordertime< (now()-interval "
				//по просьбе казани /		+ cfg.freerun_delay + " minute ) ";
				//по просьбе казани /
				
				//по просьбе казани /if (cfg.order_vendor != null) {
				//по просьбе казани /	commstr += " and ordervendor='" + cfg.order_vendor + "' ";
				//по просьбе казани /}
				//по просьбе казани /if (cfg.freerun_denied_prefix.length() > 0)
				//по просьбе казани /	commstr += " and not (meet like '"
				//по просьбе казани /			+ cfg.freerun_denied_prefix + "%')";
